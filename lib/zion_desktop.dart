              ...
              return Stack(
                children: wm.windows.map((window) {
                  return Positioned(
                    ...
                    height: window.isMaximized ? MediaQuery.of(context).size.height - 40 : window.height,
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(window.title, style: const TextStyle(color: Color(0xFF00FF41), fontSize: 12, fontFamily: 'monospace')),
                  ),
                  // تحريك النافذة
                  GestureDetector(
                    onPanUpdate: (details) => wm.updatePosition(window.id, details.delta.dx, details.delta.dy),
                    child: const Padding(padding: EdgeInsets.all(8), child: Icon(Icons.drag_indicator, color: Color(0xFF00FF41), size: 14)),
                  ),
                ],
              ),
            ),
            ...
            // مقبض تغيير الحجم
            GestureDetector(
              onPanUpdate: (details) => wm.updateSize(window.id, details.delta.dx, details.delta.dy),
              child: Container(height: 8, color: Colors.transparent, child: const Center(child: Icon(Icons.drag_indicator, color: Color(0xFF00FF41), size: 12))),
            ),
          ],
        ),
      ),
...
class _WindowButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: onTap,
      child: Container(width: 12, height: 12, margin: const EdgeInsets.only(left: 8), decoration: BoxDecoration(shape: BoxShape.circle, color: color)),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return CustomPaint(
          painter: MatrixRainPainter(_ctrl.value),
...
class MatrixRainPainter extends CustomPainter {
  ...
  MatrixRainPainter(this.time) {
    if (_columns.isEmpty) for (int i = 0; i < 100; i++) _columns.add(_RainColumn(_random));
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final col in _columns) {
      ...
      for (final char in col.chars) {
        ...
        final tp = TextPainter(text: TextSpan(text: char.char, style: TextStyle(color: paint.color, fontSize: 12, fontFamily: 'monospace')), textDirection: TextDirection.ltr);
        tp.layout();
        tp.paint(canvas, Offset(col.x, char.y));
      }
    }
  }
...
class _RainColumn {
  ...
  void update(double time, double maxHeight) {
    for (final char in chars) { char.y += speed; char.opacity = (0.05 + 0.15 * (char.y / maxHeight)).clamp(0.02, 0.2); if (char.y > maxHeight) { char.y = 0; char.char = 'ابتثجحخدذرزسشصضطظعغفقكلمنهوي'[_random.nextInt(36)]; } }
  }
}
