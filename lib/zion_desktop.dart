        ...
        children: [
          Expanded(
            child: Stack(
              children: [
                const ZionWallpaper(),
                // أيقونات سطح المكتب - أدوات Kali حقيقية
                Positioned(
                  top: 20,
                  left: 20,
                ...
                Positioned(
                  ...
                  child: DesktopIconWidget(
                    ...
                    label: 'Metasploit',
                    kaliCommand: 'msfconsole -q -x "version; exit"',
                  ),
                ),
                Positioned(
                  top: 100,
    ...
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: wm.activeWindowId == window.id ? const Color(0xFF00FF41) : const Color(0xFF1A3A1A),
            width: 2,
          ),
        ...
        child: Column(
          children: [
            Container(
              height: 30,
              decoration: BoxDecoration(
                color: wm.activeWindowId == window.id
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      window.title,
                      style: const TextStyle(color: Color(0xFF00FF41), fontSize: 12, fontFamily: 'monospace'),
                    ),
                  ),
                  GestureDetector(
                    ...
                    child: const Padding(
                      ...
                      child: Icon(Icons.drag_indicator, color: Color(0xFF00FF41), size: 14),
                    ),
                  ),
                ],
              ),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Center(child: Icon(Icons.drag_indicator, color: Color(0xFF00FF41), size: 12)),
              ),
            ),
          ],
        ),
    ...
    for (final col in _columns) {
      ...
      for (final char in col.chars) {
        ...
        final tp = TextPainter(
          text: TextSpan(text: char.char, style: TextStyle(color: paint.color, fontSize: 12, fontFamily: 'monospace')),
...
class _RainColumn {
  ...
  _RainColumn(this._random)
      : x = _random.nextDouble() * 1000,
        speed = _random.nextDouble() * 2 + 1 {
    for (int i = 0; i < _random.nextInt(20) + 5; i++) {
      chars.add(_RainChar(_random, -_random.nextDouble() * 800));
    }
  }
  void update(double time, double maxHeight) {
    for (final char in chars) {
      char.y += speed;
      char.opacity = (0.05 + 0.15 * (char.y / maxHeight)).clamp(0.02, 0.2);
      if (char.y > maxHeight) {
        char.y = 0;
        char.char = 'ابتثجحخدذرزسشصضطظعغفقكلمنهوي'[_random.nextInt(36)];
      }
    }
  }
}
